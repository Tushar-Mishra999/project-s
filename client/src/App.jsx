...
function SourceSection({ name, items }) {
  return (
    <section className="source-section">
      <h2 className="source-header">{name}</h2>
      <div className="cards">
        {items.map((it, i) => (
          <Card key={`${name}-${i}`} item={it} />
        ))}
      </div>
    </section>
  );
  ...
  return (
    <div className="wrap">
      ...
      {!loading && !error && Object.keys(sources).length === 0 && (
        <div className="state">
          <div className="state-text">
            No items passed the relevance threshold across any source. Try refreshing later.
          </div>
        </div>
      )}
      ...
      {!loading && !error &&
        Object.entries(sources).map(([name, items]) => (
          <SourceSection key={name} name={name} items={items} />
        ))}
    </div>
  );
}
