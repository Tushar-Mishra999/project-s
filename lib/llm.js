...
export async function generateChatGemma({ system, messages, maxTokens = 1024 }) {
  ...
  const prompt = allMessages
    .map((m) => `<start_of_turn>${m.role === 'assistant' ? 'model' : m.role}\n${m.content}<end_of_turn>`)
    .join('\n') + '\n<start_of_turn>model\n';

  const resp = await fetch(
    `https://1524477819713748992.us-central1-142381987276.prediction.vertexai.goog/v1/projects/${project}/locations/${location}/endpoints/mg-endpoint-6e36e7de-7a34-4edd-b289-0d41d8035d63:predict`,
    {
      method: 'POST',
