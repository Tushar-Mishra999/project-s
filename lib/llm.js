...
export async function generateChatGemma({ system, messages, maxTokens = 1024 }) {
  ...
  // Gemma only supports 'user' and 'model' roles — merge system into the last user turn
  // (which contains the actual context block) so the instruction stays adjacent to the context.
  const allMessages = messages.map((m) => ({ ...m }));
  if (system) {
    const lastUserIdx = allMessages.findLastIndex((m) => m.role === 'user');
    if (lastUserIdx !== -1) {
      allMessages[lastUserIdx].content = `${system}\n\n${allMessages[lastUserIdx].content}`;
    }
  }
  ...
  const full = (typeof pred === 'string' ? pred : pred?.content || pred?.output || '').trim();
  // The :predict endpoint echoes the full prompt — extract only the generated part
  // which appears after the last <start_of_turn>model marker.
  const marker = '<start_of_turn>model';
  const idx = full.lastIndexOf(marker);
  const generated = idx !== -1 ? full.slice(idx + marker.length) : full;
  return generated.replace(/<end_of_turn>[\s\S]*$/i, '').trim();
}
