// RAG pipeline: enrich chunks (Gemini), embed (Google text-embedding-004 via Vertex AI REST), retrieve + rerank (Gemini).
...
export async function embedText(text, model = 'text-embedding-004', inputType) {
  ...
  if (!resp.ok) throw new Error(`Vertex AI embed error ${resp.status}: ${await resp.text()}`);
