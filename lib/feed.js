...
import { generateText, generateTextWithSearch, generateChatGLM } from './llm.js';
...
async function scrapeViaLLMSearch(src, limit) {
  console.log(`[llm-search] both RSS attempts failed for ${src.name} — falling back to GLM-5 search`);
  const domain = new NodeURL(src.url).hostname.replace(/^www\./, '');

  const prompt = `Find the ${limit} most recent news articles or blog posts published on ${src.name} (${src.url}).
...
- Return up to ${limit} results`;
  ...
  const text = await generateChatGLM({
    messages: [{ role: 'user', content: prompt }],
    maxTokens: 1024,
  });

  console.log(`[llm-search] ${src.name} GLM-5 raw response:\n${text}`);

  // 1. Parse GLM-5's JSON text response
  try {
    ...
    if (articles.length > 0) {
      console.log(`[llm-search] ${src.name}: got ${articles.length} article(s) from JSON response`);
      return articles;
    }
  } catch (jsonErr) {
    console.warn(`[llm-search] ${src.name}: JSON parse failed (${jsonErr.message}), trying URL regex…`);
  }

  // 2. Regex: extract any article URLs mentioned in GLM-5's response
  ...
  if (fromRegex.length > 0) {
    console.log(`[llm-search] ${src.name}: got ${fromRegex.length} article(s) from URL regex`);
    return fromRegex;
  }

  console.warn(`[llm-search] ${src.name}: no articles found.`);
  return [];
}
...
async function scrapeIndexRss(src, limit) {
  ...
  try {
    ...
    try {
      ...
      return scrapeViaLLMSearch(src, limit);
    }
  }
...
export async function runFeedPipeline(config) {
  ...
  for (const src of config.sources) {
    ...
    let articles;
    try {
      articles = await scrapeIndexRss(src, limit);
    } catch (err) {
