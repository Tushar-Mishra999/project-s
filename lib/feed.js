...
export async function runFeedPipeline(config) {
  ...
  for (const src of config.sources) {
    ...
    console.log(`[${src.name}] scraping index via ${useRss ? 'RSS' : 'Firecrawl'}…`);
    let raw;
    try {
      raw = useRss ? await scrapeIndexRss(src.rss) : await scrapeIndex(src.url);
    } catch (err) {
    ...
    for (const it of picked) {
      // Prefer RSS-provided body (no extra fetch). Fallback to Firecrawl.
      let body = it.body || '';
      if (!body) {
        try {
