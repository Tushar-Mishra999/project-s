...
async function scrapeViaGeminiSearch(src, limit, model) {
  ...
  const prompt = `Search for the ${limit} most recent news articles or blog posts published on ${src.name} (${src.url}).

Return ONLY a JSON array with no markdown fences and no extra text:
[{"title":"article headline","url":"https://..."}]

Rules:
- Only include direct article/post URLs from ${domain}
- No homepage, category, tag, or author pages
- If fewer than ${limit} articles are found, return what you have`;

  const { text, groundingChunks } = await generateTextWithSearch({ model, user: prompt, maxTokens: 1024 });

  // Prefer grounding chunks — they are the actual URLs Gemini retrieved
  const fromChunks = groundingChunks
    .map((c) => ({ title: (c.web?.title || '').trim(), url: (c.web?.uri || '').trim() }))
    .filter((a) => a.title && a.url && a.url.includes(domain))
    .slice(0, limit);
  ...
  // Fall back to parsing Gemini's JSON text response
  ...
  const articles = parsed
    ...
    .slice(0, limit);
  console.log(`[gemini-search] ${src.name}: got ${articles.length} article(s) from text response`);
  return articles;
}
