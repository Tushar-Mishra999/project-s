...
async function scoreWorkletRelevance(items, model) {
  ...
  for (const entry of arr) {
    ...
    const rawLabel = String(entry.relevance || '').trim();
    // Normalise to title-case ("high" -> "High") and validate.
  ...
  if (nullCount > 0) console.warn(`[worklet-relevance] ${nullCount}/${items.length} items left unscored`);
  return out;
}
