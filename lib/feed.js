...
// Fetch raw RSS bytes using the same URL-string style as rss-parser's parseURL
// (no Accept-Encoding so the server returns plain uncompressed text, matching
// what parseURL receives), then sanitize malformed entities before parsing.
function fetchRssRaw(rssUrl) {
  return new Promise((resolve, reject) => {
    const lib = rssUrl.startsWith('https:') ? https : http;
    console.log(`[rss-raw] GET ${rssUrl} headers=${JSON.stringify(RSS_HEADERS)}`);
    const req = lib.get(rssUrl, { headers: RSS_HEADERS }, (res) => {
      console.log(`[rss-raw] response status=${res.statusCode} content-type="${res.headers['content-type']}" content-encoding="${res.headers['content-encoding'] || 'none'}"`);
      if (res.statusCode >= 300 && res.statusCode < 400 && res.headers.location) {
        console.log(`[rss-raw] redirect → ${res.headers.location}`);
        req.destroy();
      ...
      res.on('end', () => {
        ...
        console.log(`[rss-raw] received ${raw.length} chars, first 300:\n${raw.slice(0, 300)}`);
        const sanitized = sanitizeXml(raw);
        console.log(`[rss-raw] sanitized first 300:\n${sanitized.slice(0, 300)}`);
        resolve(sanitized);
      });
      res.on('error', reject);
    });
    req.setTimeout(30000, () => req.destroy(new Error('RSS fetch timeout')));
    req.on('error', (err) => { console.error(`[rss-raw] request error: ${err.message}`); reject(err); });
  });
}
